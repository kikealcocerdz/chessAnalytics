...
def signup():
    ...
    with open(str(user_chess) + "/key_user.pem", "wb") as f:
        f.write(
            key_user.private_bytes(
                encoding=serialization.Encoding.PEM,
                format=serialization.PrivateFormat.TraditionalOpenSSL,
                encryption_algorithm=serialization.BestAvailableEncryption(clave_user),
            )
        )
...
def claves():
    ...
    with open("AC1/keyac.pem", "wb") as f:
        f.write(
            autoridad_key.private_bytes(
                encoding=serialization.Encoding.PEM,
                format=serialization.PrivateFormat.TraditionalOpenSSL,
                encryption_algorithm=serialization.BestAvailableEncryption(
                    clave_autoridad
                ),  # esto debería sacarse de un fichero de configuración
            )
        )
...
def request_user():
    ...
    # Escribir en disco.
    key_filepath = (
        "/home/kikealcocerdz/Documents/year-3/Criptografía-Seguridad-Informática/chessAnalytics/AC1/solicitudes/"
        + str(user)
        + "_csr.pem"
    )
...
def firma():
    ...
    while not os.path.isfile(
        "AC1/nuevoscerts/" + NUMERO_SERIAL_DE_LA_AUTORIDAD + ".pem"
    ):
        pass
    ...
    os.rename(
        "AC1/nuevoscerts/" + NUMERO_SERIAL_DE_LA_AUTORIDAD + ".pem",
        str(user) + "/" + str(user) + "certificado.pem",
    )
    ...
    info_firma = (
        "stats_blitz: {stats_blitz}, stats_bullet: {stats_bullet}, stats_daily: {stats_daily}, stats_rapid: {stats_rapid}"
    ).encode("utf-8")
        ...
        return jsonify(
            {
                ...
                "token": 1,
                "user_chess": user_decrypted.decode(),
            }
        )
